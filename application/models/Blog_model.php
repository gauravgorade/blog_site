...
class Blog_model extends CI_Model
{
    ...
    function get_special_cat_stable()
    {
        $this->db->select('*');
        $this->db->from("blog_category");
        $this->db->where('special_status', "1");
    ...
    function last_blog()
     {
         ...
         $this->db->from('blog_tbl');
         $this->db->where('blog_status', 1);
         $this->db->order_by("blog_tbl.blog_id", "DESC");
         $this->db->limit(1);
         $query = $this->db->get();
         $result = $query->result();
         return $result;  
